...
const APX_MCP_DESCRIPTION: &str = "⚠️ apply_patch is BANNED in this session — your only file-edit tool is THIS one (tool name in your list: `mcp__apx__apx`; `mcp__apx__peek` is my read-only twin for line numbers — never hand-count exec output). ONE atomic script for ALL edits of the whole task; rejection changes NOTHING (fix per diagnostic, retry); never whole-file rewrites for local changes.\nGrammar: `in PATH` / `new PATH` (create); `tsel FROM_LINE \"TEXT\" [N]` = first N exact 1-line matches; `bsel \"START\" \"END\"` = one fragment span (replaces the literal, never whole lines); `rsel S:E` = complete lines; `sel LINE S:E`; then `type \"TEXT\"` or `type <<PATCH` heredoc (PATCH after last content line); `mv DEST` after `in`; `rm`/`del`; `copy`/`cut`/`paste`; `commit`.\nRules: line numbers frozen baseline — never adjust for earlier edits; inserts unselectable until `commit`; paths from `cwd` (default `.`) inside `root`; selectors double-quoted (`\\\"` escaped), single quotes invalid.\nExample (batch 2 files + heredoc): `in a.go\ntsel 4 \"func oldName()\"\ntype \"func newName()\"\nin b.go\nrsel 12:14\ntype <<PATCH\nnew body\nPATCH\ncommit`.";

const PEEK_MCP_DESCRIPTION: &str = "Read-only twin of the apx edit tool (edit twin `mcp__apx__apx`; this one usually `mcp__apx__peek`). Cheap, bounded, minimal tokens: prints targeted regions with 1-based line numbers instead of full files; those numbers are exact, drift-free coordinates to copy straight into apx scripts. Prefer this over exec `cat`/`nl` for reading file regions — never batch `nl`/`cat` over files. Selectors: `in PATH`; `sel LINE S:E`; `tsel FROM_LINE \"TEXT\" [N]`; `bsel \"START\" \"END\"`; `rsel S:E`. Tip — several files in ONE script: `in a.go\nrsel 1:30\nin b.go\nrsel 1:40`.";
        ...
        "tools": [
            {
                ...
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "script": {"type": "string", "description": "Use THIS tool for every file edit — apply_patch is banned; one atomic script for the whole task."},
                        "root": {"type": "string"},
                        "cwd": {"type": "string"}
                    },
                    "required": ["script"]
