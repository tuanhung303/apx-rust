...
const APX_MCP_DESCRIPTION: &str = "• HIGH-EFFICIENCY edit tool — fewest calls, minimal params: ONE atomic script for ALL edits of the whole task (never the apply_patch tool — create/delete files with `new PATH`/`rm`); rejection changes NOTHING (fix per diagnostic, retry); never whole-file rewrites for local changes.\n• Selectors: `in PATH` (existing) / `new PATH` (create); `tsel FROM_LINE \"TEXT\" [N]` first N exact 1-line matches; `bsel \"START\" \"END\"` one fragment span (replaces the literal, never whole lines); `rsel S:E` complete lines; `sel LINE S:E`.\n• Content: `type \"TEXT\"` or `type <<PATCH` heredoc (PATCH after last content line); also `mv DEST` (after `in`), `rm`/`del`, `copy`/`cut`/`paste`, `commit`.\n• Coordinates: line numbers frozen baseline — never adjust for earlier edits; inserts unselectable until `commit`; paths from `cwd` (default `.`) inside `root`; selectors double-quoted (`\\\"` escaped), single quotes invalid.\n• Example (batch 2 files + heredoc): `in a.go\ntsel 4 \"func oldName()\"\ntype \"func newName()\"\nin b.go\nrsel 12:14\ntype <<PATCH\nnew body\nPATCH\ncommit`.\n• Line numbers: read them from the `peek` tool — never hand-count from exec output.";

const PEEK_MCP_DESCRIPTION: &str = "• Function: Read-only viewer using apx selectors (`in`, `sel`, `tsel`, `bsel`, `rsel`) — cheap, bounded, minimal tokens.\n• Output: Prints targeted regions with 1-based line numbers instead of full files.\n• Benefit: Output line numbers double as exact, drift-free coordinates directly copyable into apx scripts (cheaper than `nl -ba`).\n• Prefer this over exec `cat`/`nl` for reading file regions — never batch `nl`/`cat` over files.\n• Tip: Read several files in ONE script: `in a.go\nrsel 1:30\nin b.go\nrsel 1:40`.";
